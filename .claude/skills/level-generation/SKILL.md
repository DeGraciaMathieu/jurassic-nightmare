...
| Difficulté par secteur | `src/config.js` `LEVELS` | `rex`, `cards`, `braid`, `vision` (torche), `sight` (vue rex), `patrol`/`chase` (vitesses), `doors` |
...
4. Logique : module `src/` dédié, appelé dans `update()` (`src/app.js`) en respectant l'ordre `player → doors → lures → pickup → rexes`.
