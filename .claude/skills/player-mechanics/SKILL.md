...
| Craquement de débris | `updatePlayer` → `bus.emit('decor:crunch')` | entrer sur une case de `state.decor` de type `crate`/`bones` craque (une fois par entrée, via `player.tileKey`) et alerte les prédateurs à moins de `CRUNCH_HEAR=260` px : `alert=max(alert, CRUNCH_ALERT=1.8)` + `seenC/seenR` = la case |
